...
from generate_bar_code_telegram_bot.telegram_bot.barcode_handlers import handle_message, handle_start_message

bot = telegram.Bot(token=settings.BARCODE_TELEGRAM_BOT_TOKEN)


def run_bot():
    # Initialize the Updater with the bot token
    updater = Updater(token=settings.BARCODE_TELEGRAM_BOT_TOKEN, use_context=True)
